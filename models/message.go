...
func (m *Message) AddChannelTimeout(sn string, equipment_id uint, site_id uint, channel uint){
	msg := &Message{}
	...
	msg.Category = 1
	msg.Level = 2 // warning
	msg.Title = fmt.Sprintf("设备【%s】通道【%d】超时异常", sn, channel)
...
func (m *Message) AddEquipmentError(sn string, equipment_id uint, site_id uint, content string){
	...
	msg.Category = 1
	msg.Level = 2 // warning
	msg.Title = fmt.Sprintf("设备【%s】异常", sn)
	msg.Content = content
	utilities.MyDB.Create(msg)
}
